...
5 Qu'est ce qu'est Boostrap 

Boostrap est un site internet regroupant une multitude d'outils, de codes HTML et CSS nécessaire à la création d'un site web.
...
```
strings ; array; boolean; integer; float
```
...
```texte
Une geme est une interface utilisateur graphique 
```
...
```
API est une interface de programmation, ce qui nous permet de nous connectés sont deux clés API
```
